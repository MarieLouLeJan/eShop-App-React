...
const orderIcon = <FaCartArrowDown size={20} color="#00c57d" />;
...
const Home = () => {
  ...
  useEffect(() => {
    ...
    if (orderData) {
      const orderNotCancelled = orderData.data.filter(
        (order) => order.order_states.id !== 5
      );
      setEarnings(
        orderNotCancelled.reduce((acc, curr) => (acc += curr.totalTTC), 0)
      );
