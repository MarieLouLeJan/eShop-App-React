...
import { useGetAllOrdersAdminQuery, useGetAllOrderStatesQuery } from '../../../../redux/api/orderApi';
...
const Chart = () => {
    ...
    useEffect(() => {
        if (OSData && ordersData) {
            ...
            let labs = []
            for(const status of datas) labs.push(status.title)
            setLabels(labs)
            let statusOrderArray = []
            for(const order of ordersData.data) statusOrderArray.push(order.order_states.title)
